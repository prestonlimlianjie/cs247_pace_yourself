...
app.post('/user', function (request, response){
    ...
    User.findOne({login_name:request.body.login_name}, function(err, userInfo){
        ...
        if (userInfo === null) {
            ...
            User.create({login_name:request.body.login_name, password:request.body.password, first_name: request.body.first_name, last_name: request.body.last_name, location: request.body.location, description: request.body.description, occupation: request.body.occupation}, doneCallBack);
...
app.get('/user/list', function (request, response) {
    ...
    User.find({}, {_id:1, first_name:1, last_name:1}, function (err, individualUser) {
        if (err) {
            // Query returned an error.  We pass it back to the browser with an Internal Service
            // Error (500) error code.
            console.error('Doing /user/info error:', err);
...
app.get('/user/:id', function (request, response) {
    ...
    if (mongoose.Types.ObjectId.isValid(id)){
        ...
        User.findOne({_id:id}, {_id:1, first_name:1, last_name:1, location:1, description:1, occupation:1}, function(err, userInfo){
            if (err){
                // Query returned an error.  We pass it back to the browser with an Internal Service
                // Error (500) error code.
